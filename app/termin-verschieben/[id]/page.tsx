...
export default function TerminVerschiebenPage() {
  ...
  function selectDay(day: number) {
    ...
    if (isEN) {
      ...
      setSelectedDate(`${dayNamesEN[date.getDay()]}, ${MONTHS_EN[calMonth]} ${day}, ${calYear}`);
    } else {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedDate) { setError(isEN ? "Please select a date." : "Bitte wählen Sie ein Datum."); return; }
    if (!selectedTime) { setError(isEN ? "Please select a time." : "Bitte wählen Sie eine Uhrzeit."); return; }
    setError("");
    setDone(true);
  }
        ...
        <div style={{ maxWidth: 680, margin: "0 auto 64px", padding: "0 40px" }}>
          {done ? (
            <div style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 40, textAlign: "center" }}>
              ...
              <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 22, color: "var(--black)", margin: "0 0 10px" }}>{isEN ? "Appointment rescheduled" : "Termin verschoben"}</h2>
            ...
            <>
              <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: 26, color: "var(--black)", margin: "0 0 6px" }}>{isEN ? "Reschedule appointment" : "Termin verschieben"}</h1>
              <p style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", margin: "0 0 24px" }}>{isEN ? "Choose a new date and time for your appointment." : "Wählen Sie ein neues Datum und eine neue Uhrzeit für Ihren Termin."}</p>

              {/* Current booking */}
              {booking && (
                <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "16px 20px", marginBottom: 20 }}>
                  <p style={{ fontFamily: F, fontSize: 12, fontWeight: 600, color: "var(--grey-text)", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 12px" }}>{isEN ? "Current appointment" : "Aktueller Termin"}</p>
                  ...
                  <div style={{ display: "flex", gap: 20 }}>
                    ...
                    <span style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: F, fontSize: 13, color: "var(--grey-text)" }}><ClockIcon /> {booking.time}{isEN ? "" : " Uhr"}</span>
                  </div>
                </div>
              )}
              ...
              <div style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 28 }}>
                <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 18, color: "var(--black)", margin: "0 0 22px" }}>{isEN ? "Choose a new appointment" : "Neuen Termin wählen"}</h2>
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                  ...
                  {slots && (
                    <div>
                      ...
                      {slots.afternoon.length > 0 && (
                        <div>
                          <p style={{ fontFamily: F, fontSize: 12, color: "var(--grey-text)", fontWeight: 500, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "0.06em" }}>{isEN ? "Afternoon & evening" : "Nachmittag & Abend"}</p>
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <div style={{ background: "#FFF8F0", border: "1px solid #FDE8C8", borderRadius: 10, padding: "12px 14px", display: "flex", gap: 8 }}>
                    ...
                    <p style={{ fontFamily: F, fontSize: 13, color: "#92400E", margin: 0, lineHeight: 1.6 }}>
                      <strong>{isEN ? "Note:" : "Hinweis:"}</strong>{" "}
                      {isEN
                        ? "Free rescheduling up to 24 hours before the current appointment. A fee may apply for last-minute changes."
                        : "Kostenlose Umbuchung bis 24 Stunden vor dem aktuellen Termin. Bei kurzfristiger Änderung können Kosten anfallen."}
                    </p>
                  </div>
