...
const DOCS_EN = [
  { id: 1, type: "Invoice", therapist: "Dr. Sarah Müller", apptDate: "12.12.2025", fileDate: "12.12.2025", color: "#F1F5F9", iconColor: "#2D5B8D" },
  { id: 2, type: "Session confirmation", therapist: "Dr. Sarah Müller", apptDate: "01.12.2025", fileDate: "01.12.2025", color: "#F1F5F9", iconColor: "#2D5B8D" },
  { id: 3, type: "Insurance form", therapist: "Dr. Sarah Müller", apptDate: "25.11.2025", fileDate: "25.11.2025", color: "#F1F5F9", iconColor: "#2D5B8D" },
];
  ...
  const textEl = (
    <span style={{ display: "inline-flex", overflow: "hidden" }}>
      ...
      <span style={{ maxWidth: canJoin ? "220px" : 0, overflow: "hidden", opacity: canJoin ? 1 : 0, transition: "max-width 0.5s ease, opacity 0.3s 0.15s", whiteSpace: "nowrap" }}>
        {isEN ? "Join conversation" : "Zum Gespräch beitreten"}
      </span>
    </span>
  );
  ...
  return (
    <div style={{ position: "relative" }} className="tooltip-wrap">
      ...
      <div className="tooltip-box" style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, background: "#F9FAFB", color: "#111827", fontFamily: F, fontSize: 13, lineHeight: 1.6, padding: "10px 14px", borderRadius: 10, whiteSpace: "normal" as const, pointerEvents: "none", opacity: 0, transition: "opacity 0.15s", zIndex: 20, width: 280, boxShadow: "0 4px 16px rgba(0,0,0,0.10)", border: "1px solid #E5E7EB" }}>
        {isEN ? "The join link will become active shortly before your session begins. Please be patient a little longer." : "Der Beitritts-Link wird kurz vor Beginn Ihrer Sitzung aktiv. Bitte haben Sie noch etwas Geduld."}
  ...
  return (
    <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div className="bc-header" style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        {past && <span style={{ fontFamily: F, fontSize: 12, fontWeight: 500, color: "#6B7280", background: "#F3F4F6", borderRadius: 9999, padding: "3px 10px" }}>{isEN ? "Past" : "Vergangen"}</span>}
      </div>
      ...
      <div className="bc-body" style={{ display: "flex", flexDirection: "column", gap: 8, paddingLeft: 58 }}>
        ...
        <span style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", display: "flex", alignItems: "center", gap: 8 }}><span style={{ color: CTA }}><ClockIcon /></span>{b.time}{isEN ? "" : " Uhr"}</span>
        <span style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", display: "flex", alignItems: "flex-start", gap: 8 }}>
          ...
          <span style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            <span style={{ fontFamily: F, fontSize: 12, fontWeight: 600, color: CTA, background: "var(--blue-ultra-light)", borderRadius: 9999, padding: "3px 10px", alignSelf: "flex-start" }}>
              {isVorOrt ? (isEN ? "In-person appointment" : "Vor-Ort-Termin") : (isEN ? "Online session" : "Online-Sitzung")}
            </span>
      ...
      {!past && (
        <div className="bc-body" style={{ paddingLeft: 58 }}>
          ...
          {!isVorgesprach && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontFamily: F, fontSize: 12, color: canCancel ? "var(--grey-text)" : "#9CA3AF" }}>
                {canCancel
                  ? (isEN ? "Free rebooking & cancellation up to 24h before start" : "Kostenlose Umbuchung & Absage bis 24 Std. vor Beginn möglich")
                  : (isEN ? "Free cancellation no longer possible – less than 24h until appointment" : "Kostenlose Absage nicht mehr möglich – weniger als 24 Std. bis zum Termin")}
              </span>
            </div>
          )}
          ...
          <div className="booking-buttons" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {confirm ? (
              <>
                <button onClick={() => setConfirm(false)} style={{ height: 36, padding: "0 12px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
                <button onClick={() => onCancel?.(b.id)} style={{ height: 36, padding: "0 12px", borderRadius: 9999, background: "#DC2626", color: "white", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Yes, cancel" : "Ja, absagen"}</button>
              </>
            ) : (
              <button onClick={() => setConfirm(true)} style={{ height: 36, padding: "0 12px", borderRadius: 9999, background: "#FEF2F2", color: "#DC2626", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Absagen"}</button>
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "22px 24px", display: "flex", flexDirection: "column", gap: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 15, color: "var(--black)" }}>{isEN ? "Security" : "Sicherheit"}</span>
        {savedSection === "security" && <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 12, color: "#16A34A", background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 9999, padding: "2px 10px" }}>✓ {isEN ? "Saved" : "Gespeichert"}</span>}
      </div>
      ...
      <button onClick={() => setOpen(open === "password" ? null : "password")} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 0", background: "none", border: "none", borderBottom: "1px solid #F3F4F6", cursor: "pointer", textAlign: "left" as const }}>
        {iconBox(lockIcon)}
        <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 14, color: "var(--black)", flex: 1 }}>{isEN ? "Change password" : "Passwort ändern"}</span>
        {chevron(open === "password")}
      </button>
      {open === "password" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "16px 0 8px" }}>
          <div><label style={lbl}>{isEN ? "Current password" : "Aktuelles Passwort"}</label><input style={inp} type="password" placeholder="••••••••" value={pwForm.current} onChange={e => setPwForm(p => ({ ...p, current: e.target.value }))} /></div>
          <div className="sicherheit-pw-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><label style={lbl}>{isEN ? "New password" : "Neues Passwort"}</label><input style={inp} type="password" placeholder={isEN ? "At least 8 characters" : "Mindestens 8 Zeichen"} value={pwForm.next} onChange={e => setPwForm(p => ({ ...p, next: e.target.value }))} /></div>
            <div><label style={lbl}>{isEN ? "Confirm" : "Bestätigen"}</label><input style={inp} type="password" placeholder={isEN ? "Repeat" : "Wiederholen"} value={pwForm.confirm} onChange={e => setPwForm(p => ({ ...p, confirm: e.target.value }))} /></div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setOpen(null); setPwForm({ current: "", next: "", confirm: "" }); saved("security"); }} style={{ height: 36, padding: "0 20px", borderRadius: 9999, background: "var(--cta)", color: "white", border: "none", fontFamily: "'Poppins', sans-serif", fontWeight: 500, fontSize: 13, cursor: "pointer" }}>{isEN ? "Save" : "Speichern"}</button>
            <button onClick={() => setOpen(null)} style={{ height: 36, padding: "0 14px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: "'Poppins', sans-serif", fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
          </div>
        </div>
      )}
      ...
      <button onClick={() => setOpen(open === "email" ? null : "email")} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 0", background: "none", border: "none", cursor: "pointer", textAlign: "left" as const }}>
        {iconBox(mailIcon)}
        <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 14, color: "var(--black)", flex: 1 }}>{isEN ? "Change email address" : "E-Mail-Adresse ändern"}</span>
        {chevron(open === "email")}
      </button>
      {open === "email" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: "4px 0 8px" }}>
          <div><label style={lbl}>{isEN ? "New email address" : "Neue E-Mail-Adresse"}</label><input style={inp} type="email" placeholder={isEN ? "new@example.com" : "neu@beispiel.at"} value={emailForm.email} onChange={e => setEmailForm(p => ({ ...p, email: e.target.value }))} /></div>
          <div><label style={lbl}>{isEN ? "Password for confirmation" : "Passwort zur Bestätigung"}</label><input style={inp} type="password" placeholder="••••••••" value={emailForm.password} onChange={e => setEmailForm(p => ({ ...p, password: e.target.value }))} /></div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setOpen(null); setEmailForm({ email: "", password: "" }); saved("security"); }} style={{ height: 36, padding: "0 20px", borderRadius: 9999, background: "var(--cta)", color: "white", border: "none", fontFamily: "'Poppins', sans-serif", fontWeight: 500, fontSize: 13, cursor: "pointer" }}>{isEN ? "Save" : "Speichern"}</button>
            <button onClick={() => setOpen(null)} style={{ height: 36, padding: "0 14px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: "'Poppins', sans-serif", fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
          </div>
        </div>
      )}
...
function ProfilSettings({ user, onLogout }: { user: { name: string; email: string }; onLogout: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 18, color: "var(--black)", margin: 0 }}>{isEN ? "My Profile" : "Mein Profil"}</h2>
        ...
        <div className="profil-col-left" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "22px 24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <span style={{ fontFamily: F, fontWeight: 700, fontSize: 15, color: "var(--black)" }}>{isEN ? "Personal data" : "Persönliche Daten"}</span>
              {savedSection === "personal" && <span style={{ fontFamily: F, fontSize: 12, color: "#16A34A", background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 9999, padding: "2px 10px" }}>✓ {isEN ? "Saved" : "Gespeichert"}</span>}
            </div>
            {editPersonal ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 16 }}>
                <div className="profil-personal-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div><label style={lbl}>{isEN ? "First name" : "Vorname"}</label><input style={inp} value={personal.vorname} onChange={e => setPersonal(p => ({ ...p, vorname: e.target.value }))} /></div>
                  <div><label style={lbl}>{isEN ? "Last name" : "Nachname"}</label><input style={inp} value={personal.nachname} onChange={e => setPersonal(p => ({ ...p, nachname: e.target.value }))} /></div>
                </div>
                <div><label style={lbl}>{isEN ? "Email address" : "E-Mail-Adresse"}</label><input style={inp} type="email" value={personal.email} onChange={e => setPersonal(p => ({ ...p, email: e.target.value }))} /></div>
                <div><label style={lbl}>{isEN ? "Phone number" : "Telefonnummer"}</label><input style={inp} placeholder="+43 123 456 789" value={personal.phone} onChange={e => setPersonal(p => ({ ...p, phone: e.target.value }))} /></div>
                <div><label style={lbl}>{isEN ? "Date of birth" : "Geburtsdatum"}</label><input style={inp} type="date" value={personal.birthdate} onChange={e => setPersonal(p => ({ ...p, birthdate: e.target.value }))} /></div>
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  <button onClick={() => { setEditPersonal(false); saved("personal"); }} style={{ height: 36, padding: "0 20px", borderRadius: 9999, background: CTA, color: "white", border: "none", fontFamily: F, fontWeight: 500, fontSize: 13, cursor: "pointer" }}>{isEN ? "Save" : "Speichern"}</button>
                  <button onClick={() => setEditPersonal(false)} style={{ height: 36, padding: "0 14px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
                </div>
              </div>
            ) : (
              <div style={{ marginTop: 8 }}>
                {row(isEN ? "First name" : "Vorname", personal.vorname)}
                {row(isEN ? "Last name" : "Nachname", personal.nachname)}
                {row(isEN ? "Email address" : "E-Mail-Adresse", personal.email)}
                {row(isEN ? "Phone number" : "Telefonnummer", personal.phone)}
                {row(isEN ? "Date of birth" : "Geburtsdatum", personal.birthdate)}
          ...
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "22px 24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <span style={{ fontFamily: F, fontWeight: 700, fontSize: 15, color: "var(--black)" }}>{isEN ? "Insurance" : "Versicherung"}</span>
              {savedSection === "versicherung" && <span style={{ fontFamily: F, fontSize: 12, color: "#16A34A", background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 9999, padding: "2px 10px" }}>✓ {isEN ? "Saved" : "Gespeichert"}</span>}
            </div>
            {editVersicherung ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
                <div>
                  <label style={lbl}>{isEN ? "Insurance type" : "Versicherungsart"}</label>
                  {(isEN ? [["gesetzlich", "Public insurance"] as const, ["privat", "Private insurance"] as const, ["selbst", "Self-payer"] as const] : [["gesetzlich", "Gesetzlich versichert"] as const, ["privat", "Privat versichert"] as const, ["selbst", "Selbstzahler"] as const]).map(([val, label]) => (
                ...
                {versicherung === "gesetzlich" && (
                  <div>
                    <label style={lbl}>{isEN ? "Health insurance fund" : "Krankenkasse"}</label>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => { setEditVersicherung(false); saved("versicherung"); }} style={{ height: 36, padding: "0 20px", borderRadius: 9999, background: CTA, color: "white", border: "none", fontFamily: F, fontWeight: 500, fontSize: 13, cursor: "pointer" }}>{isEN ? "Save" : "Ändern"}</button>
                  <button onClick={() => setEditVersicherung(false)} style={{ height: 36, padding: "0 14px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
                </div>
              </div>
            ) : (
              <div style={{ marginTop: 8 }}>
                {row(isEN ? "Insurance type" : "Versicherungsart", isEN
                  ? (versicherung === "gesetzlich" ? "Public insurance" : versicherung === "privat" ? "Private insurance" : "Self-payer")
                  : (versicherung === "gesetzlich" ? "Gesetzlich versichert" : versicherung === "privat" ? "Privat versichert" : "Selbstzahler"))}
                {versicherung === "gesetzlich" && row(isEN ? "Health insurance fund" : "Krankenkasse", kasse)}
          ...
          {/* Datenschutz */}
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "22px 24px" }}>
            <span style={{ fontFamily: F, fontWeight: 700, fontSize: 15, color: "var(--black)", display: "block", marginBottom: 16 }}>{isEN ? "Privacy" : "Datenschutz"}</span>
            {(isEN ? [
              { label: "Privacy policy", href: "/datenschutz" },
              { label: "Request data export", href: "#" },
            ] : [
            ...
            <div style={{ paddingTop: 16 }}>
              {!deleteConfirm ? (
                <button onClick={() => setDeleteConfirm(true)} style={{ background: "none", border: "none", fontFamily: F, fontSize: 13, color: "#9CA3AF", cursor: "pointer", padding: 0, textDecoration: "underline" }}>
                  {isEN ? "Delete account" : "Konto löschen"}
                </button>
              ) : (
                <div style={{ background: "#FEF2F2", borderRadius: 12, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
                  <p style={{ fontFamily: F, fontSize: 13, color: "#DC2626", margin: 0 }}>{isEN ? "Your account and all data will be irreversibly deleted. Do you wish to proceed?" : "Ihr Konto und alle Daten werden unwiderruflich gelöscht. Möchten Sie fortfahren?"}</p>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={onLogout} style={{ height: 34, padding: "0 14px", borderRadius: 9999, background: "#DC2626", color: "white", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Yes, delete" : "Ja, löschen"}</button>
                    <button onClick={() => setDeleteConfirm(false)} style={{ height: 34, padding: "0 14px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: F, fontSize: 13, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="profil-col-right" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "28px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, textAlign: "center" }}>
            ...
            <div>
              ...
              <span style={{ fontFamily: F, fontSize: 12, color: CTA, background: "var(--blue-ultra-light)", borderRadius: 9999, padding: "3px 12px", fontWeight: 500 }}>{isEN ? "Patient" : "Patient"}</span>
            </div>
          ...
          <div style={{ background: "white", borderRadius: 16, border: "1px solid #EEF2F7", padding: "22px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
            <span style={{ fontFamily: F, fontWeight: 700, fontSize: 15, color: "var(--black)" }}>{isEN ? "Notifications" : "Benachrichtigungen"}</span>
            {(Object.entries(isEN ? {
              terminbestaetigung: "Appointment confirmation by email",
              erinnerung: "Appointment reminder 24h beforehand",
              nachrichten: "Messages from specialists",
              plattform: "Platform notifications",
            } : {
...
export default function ProfilPage() {
  ...
  const [now, setNow] = useState(() => new Date());
  const [openDoc, setOpenDoc] = useState<typeof DOCS_DE[0] | null>(null);
  ...
  function printDoc(doc: typeof DOCS_DE[0]) {
...
<div class="footer">
  ...
  const DOCS = isEN ? DOCS_EN : DOCS_DE;
  const TABS: { key: Tab; label: string; icon: React.ReactNode; badge?: number }[] = [
    { key: "dashboard", label: "Dashboard", icon: <HomeIcon /> },
    { key: "termine", label: isEN ? "My appointments" : "Meine Termine", icon: <CalIcon /> },
    { key: "nachrichten", label: isEN ? "Messages" : "Nachrichten", icon: <MsgIcon />, badge: unreadCount },
    { key: "dokumente", label: isEN ? "Documents" : "Dokumente", icon: <DocIcon /> },
    { key: "profil", label: isEN ? "Profile" : "Profil", icon: <UserIcon /> },
  ];
    ...
    <>
      ...
      <main style={{ background: "#F4F7FB", minHeight: "calc(100vh - 72px)" }}>
          ...
          <div className="profil-banner" style={{
            ...
            <div className="profil-banner-inner" style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 56px", maxWidth: 560 }}>
              <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: 30, color: "var(--black)", margin: "0 0 10px", lineHeight: 1.2 }}>
                {greeting(isEN)}, {getFirstName(user)}
              </h1>
              <p style={{ fontFamily: F, fontSize: 14, color: "#374151", margin: 0, lineHeight: 1.7 }}>
                {isEN ? "Great to have you back. Here you'll find everything about your appointments and messages." : "Schön, dass Sie wieder da sind. Hier finden Sie alles Wichtige rund um Ihre Termine und Nachrichten."}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="profil-wrapper" style={{ maxWidth: 1440, margin: "0 auto", padding: "0 40px", display: "flex", flexDirection: "column", gap: 0 }}>
            ...
            <div className="profil-info-strip" style={{ background: "#F2F4F7", border: "1px solid #E4E7EC", borderRadius: "12px 12px 0 0", borderBottom: "none", padding: "12px 18px 28px", display: "flex", alignItems: "flex-start", gap: 10 }}>
              ...
              <p style={{ fontFamily: F, fontSize: 14, color: "#374151", margin: 0 }}>
                <span style={{ color: BRAND, fontWeight: 600 }}>{isEN ? "Appointment changes & cancellations:" : "Terminänderung & Stornierung:"}</span>{" "}
                {isEN ? "Free rebooking up to 24h before start. Late cancellations may incur fees. Cancellations by the specialist will be refunded." : "Kostenlose Umbuchung bis 24 Std. vor Beginn. Bei kurzfristiger Absage können Kosten anfallen. Absagen durch die Fachkraft werden erstattet."}
              </p>
            </div>
        ...
        <div className="profil-wrapper profil-content" style={{ maxWidth: 1440, margin: "0 auto", padding: "28px 40px 64px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="dashboard-grid">
                ...
                <div className="db-termin" style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 24 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 16, color: "var(--black)", margin: 0 }}>{isEN ? "Next appointment" : "Nächster Termin"}</h2>
                    <button onClick={() => setTab("termine")} style={{ fontFamily: F, fontSize: 13, color: CTA, background: "none", border: "none", cursor: "pointer", padding: 0, fontWeight: 500 }}
                      onMouseEnter={e => (e.currentTarget.style.textDecoration = "underline")}
                      onMouseLeave={e => (e.currentTarget.style.textDecoration = "none")}
                    >{isEN ? "Show all" : "Alle anzeigen"}</button>
                  </div>
                  {next ? (
                    <>
                          ...
                          return (
                            <>
                              {[
                                ...
                                { icon: <ClockIcon />, content: <span style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)" }}>{next.time}{isEN ? "" : " Uhr"}</span> },
                                {
                                  icon: nextIsVorOrt ? <MapPinIcon /> : <MonitorIcon />,
                                  content: (
                                    <span style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                      <span style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)" }}>{nextIsVorOrt ? (isEN ? "In-person appointment" : "Vor-Ort-Termin") : (isEN ? "Online session" : "Online-Sitzung")}</span>
                            ...
                            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                              ...
                              {cancelConfirm === next.id ? (
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button onClick={() => setCancelConfirm(null)} style={{ height: 40, padding: "0 12px", borderRadius: 9999, background: "#F3F4F6", color: "#374151", border: "none", fontFamily: F, fontSize: 14, cursor: "pointer" }}>{isEN ? "Cancel" : "Abbrechen"}</button>
                                  <button onClick={() => handleCancel(next.id)} style={{ height: 40, padding: "0 12px", borderRadius: 9999, background: "#DC2626", color: "white", border: "none", fontFamily: F, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>{isEN ? "Yes, cancel" : "Ja, absagen"}</button>
                                </div>
                              ) : (
                                <button
                                ...
                                >
                                  {isEN ? "Cancel appointment" : "Absagen"}
                                </button>
                              )}
                            </div>
                          </>
                ...
                <div className="db-dokumente" style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 24 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 16, color: "var(--black)", margin: 0 }}>{isEN ? "My documents" : "Meine Dokumente"}</h2>
                    <button onClick={() => setTab("dokumente")} style={{ fontFamily: F, fontSize: 14, color: CTA, fontWeight: 500, background: "none", border: "none", cursor: "pointer" }}>{isEN ? "Show all" : "Alle anzeigen"}</button>
                  </div>
                  {DOCS.map(doc => (
                ...
                <div className="db-nachrichten" style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 24, display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 16, color: "var(--black)", margin: 0 }}>{isEN ? "Latest message" : "Neueste Nachricht"}</h2>
                    <button onClick={() => setTab("nachrichten")} style={{ fontFamily: F, fontSize: 14, color: CTA, fontWeight: 500, background: "none", border: "none", cursor: "pointer" }}>{isEN ? "Show all" : "Alle anzeigen"}</button>
                  </div>
                  {threads[0] ? (
                    ...
                    <p style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", margin: 0 }}>{isEN ? "No messages yet" : "Noch keine Nachrichten"}</p>
                  )}
                  ...
                  >
                    <MsgIcon /> {isEN ? "Open messages" : "Nachrichten öffnen"}
                  </button>
                </div>
                    ...
                    >
                      <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontFamily: F, fontWeight: 500, fontSize: 14 }}>
                          {lastTherapist ? (isEN ? `Book again with ${lastTherapist.therapistName.split(" ").slice(-1)[0]}` : `Erneut bei ${lastTherapist.therapistName.split(" ").slice(-1)[0]} buchen`) : (isEN ? "Find a specialist" : "Fachkraft finden")}
                        </span>
                      </span>
              ...
              <div style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: "var(--black)", margin: 0 }}>{isEN ? "Need help?" : "Brauchen Sie Hilfe?"}</p>
                    <p style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", margin: 0 }}>{isEN ? "We're here for you." : "Wir sind für Sie da."}</p>
                  </div>
                </div>
          ...
          {tab === "termine" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div className="termine-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 18, color: "var(--black)", margin: 0 }}>{isEN ? "My appointments" : "Meine Termine"}</h2>
                <div style={{ display: "flex", background: "#F3F4F6", borderRadius: 9999, padding: 4, gap: 2 }}>
                  {(["anstehend", "vergangene"] as const).map(v => (
                    <button key={v} onClick={() => setTermineView(v)} style={{ height: 34, padding: "0 16px", borderRadius: 9999, border: "none", fontFamily: F, fontSize: 13, fontWeight: 500, cursor: "pointer", background: termineView === v ? "white" : "transparent", color: termineView === v ? CTA : "var(--grey-text)", boxShadow: termineView === v ? "0 1px 4px rgba(0,0,0,0.08)" : "none", transition: "all 0.15s" }}>
                      {v === "anstehend" ? (isEN ? "Upcoming" : "Anstehend") : (isEN ? "Past" : "Vergangene")}
                    </button>
                  ))}
                </div>
              </div>
              {termineView === "anstehend" ? (
                ...
                past.length === 0 ? (
                  <div style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", padding: 48, textAlign: "center" }}>
                    <p style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", margin: 0 }}>{isEN ? "No past appointments." : "Keine vergangenen Termine."}</p>
                  </div>
                ) : (
            ...
            <div style={{ display: "grid", gridTemplateColumns: openThread ? "300px 1fr" : "1fr", gap: 16, alignItems: "start" }} className="nachrichten-grid">
              ...
              <div className={openThread ? "chat-thread-list chat-thread-list--hidden" : "chat-thread-list"} style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", overflow: "hidden" }}>
                <div style={{ padding: "18px 20px", borderBottom: "1px solid #EEF2F7" }}>
                  <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 16, color: "var(--black)", margin: 0 }}>{isEN ? "Messages" : "Nachrichten"}</h2>
                </div>
                {threads.length === 0 ? (
                  <div style={{ padding: "32px 20px", textAlign: "center" }}>
                    <p style={{ fontFamily: F, fontSize: 13, color: "var(--grey-text)", margin: 0 }}>
                      {isEN ? "After your first session you will receive a summary here and can write to your specialist." : "Nach Ihrem ersten Gespräch erhalten Sie hier eine Zusammenfassung und können mit Ihrer Fachkraft schreiben."}
                    </p>
                  </div>
                ) : threads.map(th => (
                  ...
                  <div className="chat-window" style={{ background: "white", borderRadius: 20, border: "1px solid #EEF2F7", display: "flex", flexDirection: "column", height: 560 }}>
                    ...
                    <div style={{ padding: "12px 16px", borderTop: "1px solid #EEF2F7", display: "flex", gap: 10 }}>
                      <input value={draft} onChange={e => setDraft(e.target.value)}
                        ...
                        placeholder={isEN ? "Write a message…" : "Nachricht schreiben…"}
          ...
          {tab === "dokumente" && (
            <div>
              <h2 style={{ fontFamily: F, fontWeight: 700, fontSize: 18, color: "var(--black)", margin: "0 0 16px" }}>{isEN ? "My documents" : "Meine Dokumente"}</h2>
                ...
                {DOCS.map((doc, i) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontFamily: F, fontSize: 13, color: "var(--grey-text)", margin: 0 }}>{doc.therapist} · {isEN ? "Session of" : "Sitzung vom"} {doc.apptDate}</p>
                    </div>
                    <button className="doc-dl-btn" style={{ height: 36, padding: "0 14px", borderRadius: 9999, background: "var(--blue-ultra-light)", color: CTA, border: "none", fontFamily: F, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                      <DownloadIcon /> <span className="doc-dl-label">{isEN ? "Download" : "Herunterladen"}</span>
                    </button>
                  </div>
                ))}
              </div>
