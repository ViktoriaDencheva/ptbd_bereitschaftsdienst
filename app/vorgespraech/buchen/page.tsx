  ...
  return (
    <div style={{ background: "white", border: "1px solid #EBEBEB", borderRadius: 20, padding: "28px 24px", display: "flex", flexDirection: "column", gap: 20, ...(isMobile ? {} : { position: "sticky", top: 88, alignSelf: "flex-start" }) }}>
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
        <div style={{ width: 64, height: 64, borderRadius: 14, overflow: "hidden", flexShrink: 0, background: "var(--blue-ultra-light)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {[
          ...
          ...(format === "online" ? [{ icon: <svg width="15" height="15" fill="none" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2" stroke={CTA} strokeWidth="1.6"/><path d="M8 21h8M12 17v4" stroke={CTA} strokeWidth="1.6" strokeLinecap="round"/></svg>, label: "Online per Video" }]
...
function TimeChip({ time, selected, onClick }: { time: string; selected: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{ padding: "8px 16px", borderRadius: 9999, border: selected ? `2px solid ${CTA}` : "1.5px solid #E0E0E0", background: selected ? CTA : "white", color: selected ? "white" : "var(--black)", fontFamily: F, fontWeight: 500, fontSize: 14, cursor: "pointer", transition: "all 0.15s" }}
      onMouseEnter={e => { if (!selected) { e.currentTarget.style.borderColor = CTA; e.currentTarget.style.color = CTA; } }}
...
function NextButton({ onClick, disabled, label }: { onClick: () => void; disabled: boolean; label: string }) {
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      onMouseLeave={e => { if (!disabled) e.currentTarget.style.background = CTA; }}>
      {label}
...
function AuthGate() {
  return (
    <div style={{ maxWidth: 480, margin: "64px auto", padding: "0 24px 80px", textAlign: "center" }}>
      <div style={{ width: 64, height: 64, borderRadius: "50%", background: "var(--blue-ultra-light)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
        <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke={CTA} strokeWidth="2" strokeLinecap="round"/><circle cx="12" cy="7" r="4" stroke={CTA} strokeWidth="2"/></svg>
      </div>
      <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: 24, color: "var(--black)", margin: "0 0 10px" }}>Anmelden erforderlich</h1>
      <p style={{ fontFamily: F, fontSize: 15, color: "var(--grey-text)", margin: "0 0 32px", lineHeight: 1.6 }}>
        Um ein kostenloses Erstgespräch zu buchen, melde dich an oder erstelle ein Konto. Es dauert nur eine Minute.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <a href={`/anmelden?redirect=${encodeURIComponent("/vorgespraech/buchen")}`}
          style={{ display: "block", background: CTA, color: "white", borderRadius: 9999, padding: "14px 24px", fontFamily: F, fontWeight: 700, fontSize: 15, textDecoration: "none", boxShadow: "0 4px 16px rgba(45,91,141,0.25)", transition: "background 0.2s" }}
        ...
        <a href={`/registrieren?redirect=${encodeURIComponent("/vorgespraech/buchen")}`}
          style={{ display: "block", background: "white", color: CTA, border: `1.5px solid ${CTA}`, borderRadius: 9999, padding: "13px 24px", fontFamily: F, fontWeight: 600, fontSize: 15, textDecoration: "none", transition: "all 0.2s" }}
          onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "var(--blue-ultra-light)"}
          onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "white"}>
          Konto erstellen
        </a>
        <a href="/vorgespraech" style={{ fontFamily: F, fontSize: 14, color: "var(--grey-text)", textDecoration: "none", marginTop: 4 }}
...
function Confirmation({ format, selectedDate, selectedTime, isMobile, user }: {
  format: "online" | "vor-ort"; selectedDate: string; selectedTime: string; isMobile: boolean; user: AuthUser;
}) {
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: isMobile ? "32px 16px 64px" : "48px 40px 80px" }}>
      {/* Hero */}
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        <p style={{ fontFamily: F, fontSize: 15, color: "var(--grey-text)", margin: "0 0 24px", lineHeight: 1.6 }}>
          Wir freuen uns auf das Gespräch mit dir, {user.name.split(" ")[0]}.<br />Eine Bestätigung wurde an <strong style={{ color: "var(--black)" }}>{user.email}</strong> gesendet.
        </p>
        {/* Chips */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center" }}>
          {[
            { label: selectedDate },
            { label: `${selectedTime} Uhr · 30 Min.` },
            { label: format === "online" ? "Online per Video" : "Vor Ort in Wien" },
            { label: "Kostenlos" },
          ].map((chip, i) => (
            <div key={i} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "white", border: "1px solid #E8E8E8", borderRadius: 9999, padding: "8px 16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16, marginBottom: 28 }}>
        <div style={{ background: "var(--blue-ultra-light)", border: "1px solid #C8DFFF", borderRadius: 20, padding: "24px 22px" }}>
          ...
          {[
            ...
            format === "online" ? "Den Videoanruf-Link findest du rechtzeitig in deinem Profil." : "Die Adresse findest du in der Bestätigungs-E-Mail.",
                      ...
                      {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                          <button key={day} onClick={() => avail && selectDay(day)} disabled={!avail || isPast}
                            ...
                            onMouseEnter={e => { if (avail && !isSel) e.currentTarget.style.background = "#D6EBFF"; }}
                            onMouseLeave={e => { if (avail && !isSel) e.currentTarget.style.background = "var(--blue-ultra-light)"; }}>
                            {day}
                          </button>
                        );
                      })}
