...
export default function KontaktPage() {
  ...
  const THEMEN = isEN
    ? ["General question", "Finding a therapist", "Initial consultation", "Costs & insurance", "Technical issue", "Feedback & suggestions", "Other"]
    : ["Allgemeine Frage", "Therapeut*in finden", "Erstgespräch", "Kosten & Krankenkasse", "Technisches Problem", "Feedback & Anregungen", "Sonstiges"];

  const TIMELINE = isEN ? [
    { title: "Message received", desc: "We receive your enquiry and automatically confirm receipt by e-mail." },
    { title: "Personal review", desc: "Our team reads your message and forwards it to the right person." },
    { title: "Reply within 24h", desc: "You'll receive a personal response — often within just a few hours." },
    { title: "Further support", desc: "If needed, we'll guide you through the next step — consultation, referral or information." },
  ] : [
  ...
  const FAQS = isEN ? [
    { q: "How quickly will I receive a reply to my message?", a: "We aim to answer all enquiries within 24 working hours. For urgent matters, we recommend contacting us by phone." },
    { q: "Is the initial consultation really free?", a: "Yes, the initial consultation is completely free and non-binding. It lasts approx. 20 minutes and helps you find out which type of support suits you best." },
    { q: "Can I make contact anonymously?", a: "Yes, you can also contact us anonymously. If you'd like a callback or a direct reply, we'll of course need your contact details." },
    { q: "What should I do if I'm in an acute crisis?", a: "In case of acute risk to yourself or others, please call the emergency number 112 or the telephone counselling service at 142 immediately. Our platform is not designed for emergency situations." },
  ] : [
  ...
  const WARUM = isEN ? [
    { text: "We find the right support for you together", icon: "/icons/icon-orientierung.svg" },
    { text: "No waiting lists — fast, straightforward help", icon: "/icons/icon-clock.svg" },
    { text: "Our team knows the Austrian care system", iconEl: WARUM_ICON_SVG },
    { text: "Confidential, free and without obligation", icon: "/icons/lock-keyhole.svg" },
    { text: "Available Austria-wide — in all 9 states", icon: "/icons/icon-pin.svg" },
    ...
    { text: "Unser Team kennt das österreichische Versorgungssystem", iconEl: WARUM_ICON_SVG },
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center" }}>
            <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 40px", width: "100%" }}>
              <div style={{ maxWidth: 560 }}>
                <span style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: CTA_HEX, letterSpacing: "0.1em", textTransform: "uppercase" }}>{isEN ? "Contact" : "Kontakt"}</span>
                <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: 44, lineHeight: 1.2, color: "var(--black)", margin: "10px 0 16px" }}>{isEN ? <>We are here<br />for you.</> : <>Wir sind für<br />dich da.</>}</h1>
                <p style={{ fontFamily: F, fontSize: 16, color: "var(--grey-text)", lineHeight: 1.7, margin: "0 0 20px" }}>
                  {isEN
                    ? "Whether you have questions, need support or simply don't know where to start — we're here to help."
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 28 }}>
                  {(isEN
                    ? ["Personal advice on request", "Reply within 24 hours", "Free & confidential"]
                    : ["Persönliche Beratung auf Wunsch", "Antwort innerhalb von 24 Stunden", "Kostenlos & vertraulich"]
                  ).map((text, i) => (
        ...
        <div className="k-hero-mobile" style={{ display: "none", position: "relative", flexDirection: "column", justifyContent: "flex-start", minHeight: "calc(100svh - 64px)" }}>
          ...
          <div style={{ position: "relative", zIndex: 2, padding: "32px 20px 0", display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: 32, lineHeight: 1.2, color: "var(--black)", margin: 0 }}>{isEN ? "We are here for you." : "Wir sind für dich da."}</h1>
      ...
      <section style={{ background: "#F5F9FD" }} className="k-section">
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <span style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: CTA, letterSpacing: "0.1em", textTransform: "uppercase" }}>{isEN ? "Directly reachable" : "Direkt erreichbar"}</span>
            <h2 style={{ fontFamily: F, fontWeight: 700, color: "var(--black)", margin: "8px 0 0" }} className="k-h2">{isEN ? "How to reach us" : "So erreichst du uns"}</h2>
          </div>
          {/* Desktop: 5-col grid */}
      ...
      <section id="kontaktformular" style={{ background: "white" }} className="k-section k-section-lg">
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div className="k-form-row">
            {/* Form */}
            <div style={{ flex: 1 }}>
              <span style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: CTA, letterSpacing: "0.1em", textTransform: "uppercase" }}>{isEN ? "Write to us" : "Schreib uns"}</span>
              <h2 style={{ fontFamily: F, fontWeight: 700, color: "var(--black)", margin: "8px 0 28px" }} className="k-h2">{isEN ? "Contact Form" : "Kontaktformular"}</h2>
              {submitted ? (
                <div style={{ background: "var(--blue-ultra-light)", borderRadius: 20, padding: "40px 32px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16, textAlign: "center" }}>
                  ...
                  <h3 style={{ fontFamily: F, fontWeight: 700, fontSize: 22, color: "var(--black)", margin: 0 }}>{isEN ? "Message sent!" : "Nachricht gesendet!"}</h3>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div className="k-name-grid">
                    {(isEN
                    ? [{ label: "First name *", key: "vorname", placeholder: "Max" }, { label: "Last name *", key: "nachname", placeholder: "Smith" }]
                    : [{ label: "Vorname *", key: "vorname", placeholder: "Max" }, { label: "Nachname *", key: "nachname", placeholder: "Mustermann" }]
                  ).map(f => (
                  ...
                  {(isEN ? [
                    { label: "E-Mail *", key: "email", type: "email", placeholder: "max@example.com", required: true },
                    { label: "Phone", key: "telefon", type: "tel", placeholder: "+43 1 234 567", required: false, optional: true },
                  ] : [
                    ...
                    <label key={f.key} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      <span style={{ fontFamily: F, fontWeight: 500, fontSize: 13, color: "var(--black)" }}>
                        {f.label} {f.optional && <span style={{ color: "var(--grey-text)", fontWeight: 400 }}>({isEN ? "optional" : "optional"})</span>}
                      </span>
                  ...
                  <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <span style={{ fontFamily: F, fontWeight: 500, fontSize: 13, color: "var(--black)" }}>{isEN ? "Topic *" : "Thema *"}</span>
                  ...
                  <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <span style={{ fontFamily: F, fontWeight: 500, fontSize: 13, color: "var(--black)" }}>{isEN ? "Message *" : "Nachricht *"}</span>
                    <textarea value={formState.nachricht} required rows={5} onChange={e => setFormState(s => ({ ...s, nachricht: e.target.value }))} placeholder={isEN ? "Describe your concern…" : "Beschreibe dein Anliegen…"}
            ...
            <div className="k-form-sidebar" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{ borderRadius: 20, overflow: "hidden", border: "1.5px solid #EAF0FA" }}>
                <div style={{ background: CTA_HEX, padding: "22px 24px" }}>
                  <h3 style={{ fontFamily: F, fontWeight: 700, fontSize: 18, color: "white", margin: 0 }}>{isEN ? "Why contact us?" : "Warum uns kontaktieren?"}</h3>
                  <p style={{ fontFamily: F, fontSize: 13, color: "rgba(255,255,255,0.75)", margin: "4px 0 0" }}>{isEN ? "5 good reasons to write to us" : "5 gute Gründe, uns zu schreiben"}</p>
                </div>
      ...
      <section style={{ background: "#F5F9FD" }} className="k-section">
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <span style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: CTA, letterSpacing: "0.1em", textTransform: "uppercase" }}>{isEN ? "After your message" : "Nach deiner Nachricht"}</span>
            <h2 style={{ fontFamily: F, fontWeight: 700, color: "var(--black)", margin: "8px 0 0" }} className="k-h2">{isEN ? "What happens next" : "So geht es weiter"}</h2>
          </div>
          {/* Desktop timeline */}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div className="k-map-row">
            <div className="k-map-text">
              <span style={{ fontFamily: F, fontWeight: 600, fontSize: 14, color: CTA, letterSpacing: "0.1em", textTransform: "uppercase" }}>{isEN ? "Our network" : "Unser Netzwerk"}</span>
              <h2 style={{ fontFamily: F, fontWeight: 700, color: "var(--black)", margin: "8px 0 16px" }} className="k-h2">{isEN ? "Here for you across Austria" : "Österreichweit für dich da"}</h2>
              <p style={{ fontFamily: F, fontSize: 15, lineHeight: 1.7, color: "var(--grey-text)", margin: "0 0 24px" }}>
                {isEN ? "Select a state on the map to discover professionals in your region." : "Wähle ein Bundesland auf der Karte, um Fachkräfte in deiner Region zu entdecken."}
              </p>
              {selectedProvince && (
      ...
      <section style={{ background: "white" }} className="k-section k-faq-section">
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ textAlign: "center", marginBottom: 48, display: "flex", flexDirection: "column", gap: 16 }}>
            <h2 style={{ fontFamily: F, fontWeight: 600, lineHeight: 1.3, color: "var(--black)", margin: 0 }} className="k-h1-faq">{isEN ? "Frequently Asked Questions" : "Häufige Fragen"}</h2>
            <p style={{ fontFamily: F, fontWeight: 400, lineHeight: 1.5, color: "var(--grey-text)", margin: 0 }} className="k-sub-faq">
              {isEN ? "Here you'll find answers to common questions about contact and availability." : "Hier findest du Antworten auf häufige Fragen rund um Kontakt und Erreichbarkeit."}
            </p>
          </div>
